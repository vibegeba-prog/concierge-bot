...
Автоматические посты в группу ВКонтакте:
  ...
  • Вечер (20:00) — каждый будний день: закат + пожелание
...
CITY_PREP = "Санкт-Петербурге"  # предложный падеж: в Санкт-Петербурге

DISTRICT       = "Васильевский остров"
DISTRICT_PREP  = "Васильевском острове"   # на Васильевском острове
DISTRICT_DAT   = "Васильевскому острову"  # по Васильевскому острову
DISTRICT_GEN   = "Васильевского острова"  # с Васильевского острова
...
def generate_morning_post(weather: dict) -> str:
    ...
    if "дождь" in desc or "ливень" in desc or "гроза" in desc:
        templates = [
            ...
            f"Утро на {DISTRICT_PREP} начинается с дождя {emoji} +{temp}°. Если вы ещё дома — захватите непромокаемую обувь. Мы уже на месте и следим за состоянием дворов. Берегите себя!",
        ]
        ...
        templates = [
            ...
            f"Снежное утро на {DISTRICT_PREP} {emoji} +{temp}°. Мы в режиме повышенной готовности: уборка снега, посыпка песком. Выходите из дома в удобной обуви. Доброго дня!",
        ]
    elif temp >= 25:
        templates = [
            ...
            f"Жаркое утро на {DISTRICT_PREP} {emoji} +{temp}°. Если у вас есть балкон — откройте его на проветривание сейчас, до полудня. Берегите себя от перегрева!",
        ]
    elif temp >= 18:
        templates = [
            ...
            f"Прекрасное утро! {emoji} +{temp}° — такая погода, ради которой стоит проснуться пораньше. Прогуляйтесь по {DISTRICT_DAT} перед работой. Удачного дня!",
        ]
    elif temp >= 10:
        templates = [
            f"Доброе утро! {emoji} Сегодня +{temp}°, {desc}. Прохладно, но комфортно. Наденьте что-то тёплое — ветер {wind} м/с. Хорошего дня!",
            f"Утро на {DISTRICT_PREP} {emoji} +{temp}°. Прохладно, но свежо. Отличная погода для утренней прогулки с чашкой кофе. Бодрого дня!",
        ]
    elif temp >= 0:
        templates = [
            ...
            f"Прохладное утро на {DISTRICT_PREP} {emoji} +{temp}°. Тёплый кофе, тёплая куртка — и вперёд. Мы уже на месте, проверяем состояние дворов. Удачи!",
        ]
    else:
        templates = [
            ...
            f"Морозное утро на {DISTRICT_PREP} {emoji} {temp}°. Тёплый чай, шарф и хорошее настроение — лучшая защита от холода. Мы следим за чистотой и безопасностью. Доброго дня!",
        ]
...
def generate_evening_post(sunset_time: str | None) -> str:
    ...
    if sunset_time and sunset_already_passed(sunset_time):
        templates = [
            f"🌙 Вечер на {DISTRICT_PREP}. Надеемся, ваш день прошёл хорошо. Завтра встретимся снова — в 9:00. Тихой ночи!",
            f"🌃 {DISTRICT} засыпает. Если вы ещё на работе — скорее домой, тут тепло и уютно. До завтра!",
            f"🌆 Вечер в {CITY_PREP}. Окна горят тёплым светом — приятно смотреть с улицы. Отдохните хорошо. УК Мир.",
            f"⭐ Небо над Невой уже тёмное. Время чая, книги или любимого сериала. Завтра новый день. Спокойной ночи!",
            f"🌙 {DISTRICT_PREP} тихий и спокойный. Мы желаем вам тёплого пледа и сладких снов. До встречи утром!",
        ]
        post = random.choice(templates)
    ...
    if sunset_time:
        templates = [
            ...
            f"Вечерний привет с {DISTRICT_GEN}! 🌆 Закат сегодня в {sunset_time}. Прогуляйтесь по набережной — свежий воздух и красивый вид — лучшее завершение дня. Спокойной ночи!",
